	...
	)
{
	...
	while (true)
	{
		...
		// Write encoded packet to shared memory
		...
		{	
			// Added for debugging
			//std::cout << "Packet[5]" << static_cast<int>(packet[5]) << std::endl;
			//std::cout << "Packet[50]" << static_cast<int>(packet[50]) << std::endl;

			// For each encoded packet
			//fpOut.write(reinterpret_cast<char*>(packet.data()), packet.size());

			//thd_args->frame_data_info->data_size = packet.size();
        	memcpy(thd_args->data + packet_size, packet.data(), packet.size());
			packet_size += packet.size();
		}

		// Added for debugging
		std::cout << "Total size of encoded packets: " << packet_size << std::endl;

		// Total size of encoded packets
		thd_args->frame_data_info->data_size = packet_size;
		...
		if (nRead != nFrameSize) {
			std::cout << "AppEncCuda.cpp pthread_count: " << pthread_count << std::endl;
			pthread_mutex_lock(&(thd_args->mutex));
			thd_args->frame_data_info->data_size = 999999;
...
{	
	...
	try
	{
		...
		ParseCommandLine(thd_args->argc, thd_args->argv, szInFilePath, nWidth, nHeight, eFormat, szOutFilePath, encodeCLIOptions, iGpu,
		...
		{
			EncodeCuda(nWidth, nHeight, eFormat, encodeCLIOptions, cuContext, fpIn, fpOut, thd_args);
		}
